...
Command? )";
        ...
        // CREATE A NEW PRODUCT FROM OPTIONS
            ...
            int desktop = store.new_desktop(); //creates new Desktop on heap and returns index
            while(true) {
                std::cout << store.desktop(desktop) << "\n\n"; //gets the Desktop at given index calls the operator<<
                for(int i=0; i<store.num_options(); ++i)       //loops through the vecotr of periphals 
